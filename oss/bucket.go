...
func (bucket Bucket) getRequest(method, objectName string, params map[string]interface{}, options []Option,
	data io.Reader, listener ProgressListener) (*http.Request, error) {
	err := CheckObjectName(objectName)
	if err != nil {
		return nil, err
	}
	...
	if params["csize"] != nil {
		headers[HTTPHeaderContentLength] = params["csize"].(string)
		delete(params, "size")
	}
	if params["cmd5"] != nil {
		headers[HTTPHeaderContentMD5] = params["cmd5"].(string)
		delete(params, "cmd5")
	}
	...
	return bucket.Client.Conn.Request(method, bucket.BucketName, objectName, params, headers, data, 0, listener)
}
